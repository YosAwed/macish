/*
  "sjis.c" ish for OSK/Human68K(CPU68000)
  shift jis encode & decode
  Fully Collected by Ken Miyazaki
  a little change for OSK by Gigo
  1987/12/16 
  88/05/10 for UNIX (kondo)
  93/08/23 -lf (aka)
  12/09/17 for Linux (bko)
*/

#include "ish.h"

extern unsigned char ent_j8[],dct_j8[];

/*  encode & decode shift jis  */

unsigned char ent_sj[] = {
	
};

unsigned char ent_sj2[] = {
	
};

void enc_sjis(uchar *argv, int len)
{
	unsigned short sjef, i, j, k;
	unsigned int d;
	unsigned char c, du, dl;
	unsigned char *dat;

	argv[len] = '\0';
	dat = argv;
	obuf[0] = ent_j8[*dat & 0x7f];

	for (i = 1, k = 1, sjef = 0; dat < argv+len-1; ++i) {
		if ((j=8 - (i & 8 - 1)) == 0) ++dat;

		d = unschr(*dat) >> j;
		d |= unschr(*++dat) << (8 - j);

		if(j > 1) d |= unschr(*++dat) << (16 - j);

		/* extract lower 15 bits */
		d &= 0x7fff;
		/* split into 2 chars */
		du = unschr((d / 182));
		dl = unschr((d - du * 182));
		/* 1st char */
		if (sjef == 0) {
			c = unschr(ent_sj[du]);
			if ((c >= 0x80 && c <= 0x9f) || (c >= 0xe0 && c <= 0xe9)) sjef = 1;
		}
		else {
			c = unschr(ent_sj2[du]);
			sjef = 0;
		}
		obuf[k++] = c;
		/* 2nd char */
		if (sjef == 0) {
			c = unschr(ent_sj[dl]);
			if ((c >= 0x80 && c <= 0x9f) || (c >= 0xe0 && c <= 0xe9)) sjef = 1;
		}
		else {
			c = unschr(ent_sj2[dl]);
			sjef = 0;
		}
		obuf[k++] = c;
	}

	/* the last char has 7bit data */
	dl = unschr(*dat) >> 1;
	if(sjef == 0) {
		c = unschr(ent_sj[dl]);
		if((c >= 0x89 && c <= 0x9f) || (c >= 0xe0 && c <= 0xe9)) c += 0x40;
	}
	else {
		c = unschr(ent_sj2[dl]);
	}
	obuf[k++]=c;
	obuf[k]='\0';
}

unsigned char dct_sj[] = {
	
};

unsigned char dct_sj2[] = {
	
};

int dec_sjis(uchar *argv, int len)
{
	 unsigned char c, *dat;
	 unsigned short d, sjdf, i, j, k;

	if(len > 78) len=78;

	argv[len] = '\0';
	dat = obuf;
	if ((c = dct_j8[unschr(argv[0])]) == 0xff) return 0;

	*dat = c;

	/* 15bit decoding */
	for (i = 1, k = 1, sjdf = 0; k < len-1; ++i) {
		/* 1st char */
		c = unschr(argv[k++]);
		if (sjdf == 0) {
			if((c >= 0x80 && c <= 0x9f) || (c >= 0xe0 && c <= 0xe9))
			sjdf = 1;
			c = unschr(dct_sj[c]);
		}
		else {
			c = unschr(dct_sj2[c]);
			sjdf = 0;
		}
		if (c == 0xff) return 0;
		d = c * 182;
		/* 2nd char */
		c = unschr(argv[k++]);
		if (sjdf == 0) {
			if ( (c >= 0x80 && c <= 0x9f) || (c >= 0xe0 && c <= 0xe9)) sjdf = 1;
			c = unschr(dct_sj[c]);
		}
		else {
			c = unschr(dct_sj2[c]);
			sjdf = 0;
		}
		if (c == 0xff) return 0;
		if ((d += c) > 0x7fff) return 0;

		/* store to LINE */
		if ((j = (i & (8 - 1))) != 0) *dat |= d << (8 - j) & 0xff;
		*++dat = d >> j;
		if(j < 7) *++dat = d >> (8 + j);
	}

	/* the last char has 7bit data */
	c = unschr(argv[k++]);
	if (sjdf == 0) {
		if(c >= 0xc9 && c <= 0xdf) c = c + (0x100 - 0x40) & 0xff;
		c = unschr(dct_sj[c]);
	}
	else c = unschr(dct_sj2[c]);
	if (c == 0xff) return 0;
	*dat++ |= c << (8 - i & (8 - 1)) & 0xff;
	return dat-obuf;
}
